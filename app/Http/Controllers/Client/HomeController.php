...
class HomeController extends Controller
{
    public function index() {
        ...
        $product_top_review_1 =  DB::select(
            'SELECT b.*, count(a.product_id) FROM comments a, products b WHERE a.product_id = b.id and b.status = 1 GROUP BY a.product_id ORDER BY count(*) DESC LIMIT 3'
        );
        $product_top_review_2 =  DB::select(
            'SELECT b.*, count(a.product_id) FROM comments a, products b WHERE a.product_id = b.id and b.status = 1 GROUP BY a.product_id ORDER BY count(*) DESC LIMIT 3 OFFSET 3'
        );
        $product_top_review_3 =  DB::select(
            'SELECT b.*, count(a.product_id) FROM comments a, products b WHERE a.product_id = b.id and b.status = 1 GROUP BY a.product_id ORDER BY count(*) DESC LIMIT 3 OFFSET 6'
        );
